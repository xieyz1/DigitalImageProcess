...
%
%	'uniform'		Uniform random numbers inthe interval (A, B).
...
% Begin processing. Use lower(type) to protext against input
% being capitalized.
switch lower(type)
	case 'uniform'
		R = a + (b - a)*rand(M, N);
	...
	case 'salt & pepper'
		...
		% Chech to make sure that Pa + Pb is not > 1.
		if (a + b) > 1
		...
		% > a & <= (a + b). These we called (salt noise).
		X = rand(M, N);
		c = find(X <= a);
		R(c) = 0;
		u = a + b;
